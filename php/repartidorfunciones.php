...
function emailExiste($correo, $con)
{
    ...
    $sql = $con->prepare("SELECT COUNT(*) FROM repartidor WHERE correo_R = ?");
...
function placaExiste($placa,$con){

    // Prepara la consulta para seleccionar el correo electrónico
    $sql = $con->prepare("SELECT COUNT(*) FROM repartidor WHERE placa = ?");
    // Ejecuta la consulta con el correo electrónico proporcionado como parámetro
    $sql->execute([$placa]);
    // Obtiene el resultado (número de filas donde la placa coincide)
    $resultado = $sql->fetchColumn();

    // Si el resultado es mayor que cero, significa que la placa ya existe
    if ($resultado > 0) {
        return true;
    } else {
        return false;
...
function dniExiste($dni, $con)
{
    ...
    $sql = $con->prepare("SELECT COUNT(*) FROM repartidor WHERE dni_R = ?");
    $sql->execute([$dni]);
    $resultado = $sql->fetchColumn();
...
function celularExiste($celular, $con){
     ...
     // Prepara la consulta para seleccionar el número de celular
     $sql = $con->prepare("SELECT COUNT(*) FROM repartidor WHERE telefono_R = ?");
     // Ejecuta la consulta con el número de celular proporcionado como parámetro
     $sql->execute([$celular]);
     // Obtiene el resultado (número de filas donde el número de celular coincide)
     $resultado = $sql->fetchColumn();
...
*/
function registrarRepartidor(array $datos, $con)
{
    $sql = $con->prepare("INSERT INTO repartidor(dni_R, nombres_R, apellido_paterno_R, apellido_materno_R, telefono_R, fecha_nacimiento_R, 
    correo_R, contrasena_R, genero_R, direccion_R, placa, vehiculo) 
    VALUES (?,?,?,?,?,?,?,?,?,?,?,?)");
