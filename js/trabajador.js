...
function confirmarEntrega(idVenta) {
    // Mostrar una ventana modal de confirmación
    var confirmacion = confirm("¿Está seguro de que el pedido fue entregado?");
    // Si el usuario acepta la confirmación
    if (confirmacion) {
        // Llamar a la función para actualizar el pedido
        actualizarPedido(idVenta);
    }
}

function actualizarPedido(idVenta) {
    // Realizar una petición AJAX para enviar los datos al servidor
    $.ajax({
        type: 'POST',
        url: 'pedidos.php', // Archivo PHP que procesará la actualización
        data: { id_venta: idVenta, accion: 'actualizarPedido' },
